...
const CheckboxField = ({ id, labelText, onChange, bgColor, textEllipsis = false, color, ...props }) => {
  <CheckStyled bgColor={bgColor} color={color} textEllipsis={textEllipsis}>
    <Checkbox onChange={onChange} label={labelText} {...props} />;
  </CheckStyled>;
};
...
const CheckStyled = styled.div`
  background-color: ${(props) => props.bgColor || '#fff'} .bx--checkbox-label {
    width: 100%;
    padding-left: 28px;
  }
